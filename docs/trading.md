...
`max_entries_per_day`는 **자본 안전장치가 아닙니다**. 자본은 `max_positions`(4) ×
`max_alloc_per_position_krw`(200만) = 800만 ≤ 가상자본 850만으로 이미 묶여 있고, 개별
포지션 크기는 호가창 유동성이 정합니다([포지션 사이징](#포지션-사이징)). 오버나이트 누적도
일일 강제청산으로 사라졌습니다. 즉 이 값은 **노출이 아니라 회전율만** 제한합니다.
